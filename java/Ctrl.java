...
public class Ctrl {
	...
	//初期画面表示
	  ...
	  public ModelAndView home() {
		...
		{
			//変数宣言
			String syainID = form.getId_Over();
			int result = 0;
			
			//社員ID存在チェック
			int judge_Check = model.idExistJudge(syainID);
		...
		public boolean insert(@RequestBody Param form) throws ClassNotFoundException, SQLException
		{
			//変数宣言
			String syainID = form.getId_Over();
			String syainNAME = form.getName_Over();
			String syainBIRTH = form.getBirth();
			String syainAGE = form.getAge();
			String syainGENDER = form.getGender();
			String syainKIHONKYU = form.getBasic_Salary();
			String syainKOUTUHI = form.getTransport_Expens();
			boolean dist= false;
		...
		public boolean delete(@RequestBody Param form) throws ClassNotFoundException, SQLException
		{
			...
			int judge_Check = model.existCheck(syainID,syainNAME,syainBIRTH);
			
			//組み合わせチェックの結果によって処理を振り分ける
			if(judge_Check == 1)//存在する時
			{
